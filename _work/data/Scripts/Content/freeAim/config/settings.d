...
const int   FREEAIM_REUSE_PROJECTILES  = TRUE;  // Enable collection & re-using of shot projectiles
const int   FREEAIM_CUSTOM_COLLISIONS  = TRUE;  // Custom collision behaviors and hit registration
const int   FREEAIM_CRITICALHITS       = TRUE;  // Critical hits for ranged combat (e.g. head shots)
...
// FREEAIM_CUSTOM_COLLISIONS
const int   FREEAIM_COLL_PRIOR_NPC     = -1;    // After wld collision: ignre(-1), dstry(0), coll(1), dflct(2) off NPC
const int   FREEAIM_TRIGGER_COLL_FIX   = TRUE;  // Trigger collision fix (disable collision) necessary for Gothic 2 only
